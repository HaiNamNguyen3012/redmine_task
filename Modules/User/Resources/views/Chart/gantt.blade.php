        ...
        <div class="container-fluid">
            <div id="wrap-form" class="wrap-form detail-task padding30">
                <form class="" method="get" id="form">
                    <div class="row">
                        ...
                        <div class="col-md-4 col-sm-4">
                            @include('form.select.select', ["title" =>  "表示", "name" => "display", "place" => "", $option_arr = config("sys_common.find_option"), "value" => (\request()->has("display") ? \request()->get("display") : "person"), 'submit' => true, 'default' => false ])
                        </div>
                        <div class="col-md-4 col-sm-4">
                            @include('form.select.select', ["name" => "status", "title" => "ステータス", "place" => "", $option_arr = $listStatus, "value" => (\request()->has("status") ? \request()->get("status") : "") , 'submit' => true ])
                        </div>
                    </div>
                </form>
            </div>
            ...
            <div class="gantt_chart">
                ...
                <div class="list-member">
                    @foreach($data["task_for_project"] as $k => $tasks)
                        @php
                            $name = '';
                            if(!empty($data["member_task"]["user"][$k][0]->id)) {
                                $name = !empty($data["member_task"]["user"][$k][0]->name) ? $data["member_task"]["user"][$k][0]->name : $data["member_task"]["user"][$k][0]->email;
                            }else{
                                foreach($tasks as $row){
                                    $name =  !empty($row["user"]["name"]) ? $row["user"]["name"] : $row["user"]["email"];
                                    break;
                                }
                            }
                        @endphp
                        @if(!empty($name) && !empty($tasks))
                                                    ...
                                                    @if($x >= $row["point_start"] && $x <=  $row["point_end"])
                                                        ...
                                                        @if($dem == 1)
                                                            <div class="item" style="width: {{ $day * 34 }}px">
                                                                <a href="{{ route('user.task.show', ['id' => $row["id"]]) }}"
                                                                   target="_blank">
                                                                    <div
