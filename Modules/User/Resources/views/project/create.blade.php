        ...
        <div class="container-fluid">
            <form class="" method="post" id="form">
                ...
                <div id="wrap-form" class="wrap-form padding30">
                    ...
                    @endif
                    @include('form.input.text', ["title" =>  "Tên dự án", "name" => "name", "place" => "Tên dự án", "value" => ""])
                    @include('form.textarea.text', ["title" =>  "Chi tiết", "name" => "details", "place" => "Chi tiết", "value" => ""])
                    <div class="form-group mb-3">
                        <label for="">Danh sách đầu việc có thể lựa chọn</label>
                    </div>
                    @foreach(config("sys_common.task_category") as $key => $value)
                        @include('form.input.checkbox', ["name" => $key, "id" => $key, "label" => $value, "checked" => false, "value" => "1"])
                    @endforeach
                </div>
                ...
                <div id="wrap-form" class="wrap-form padding30 status-project">
                    <div class="row">
                        <div class="col-md-2 col-sm-4 col-6">
                            <p class="title"><span class="font-bold">trạng thái có thể lựa chọn</span></p>
                        </div>
                        <div class="col-md-3 col-sm-4 col-6">
                            <div class="row">
                                <div class="col-md-10 col-sm-10 col-10">
                                    <p class="title text-end">thứ tự hiển thị lựa chọn</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    @php
                        ...
                        $checkbox_name_key = "project_status_key[]";
                        ...
                        <div class="row row-status">
                            <div class="col-md-2 col-sm-4 col-6">
                                <div class="list-status">
                                    <input type="hidden" name="{{ $checkbox_name }}" value="{{ $value }}"/>
                                    <input type="hidden" name="{{ $checkbox_name_key }}" value="{{ $key }}"/>
                                    @if($key == "unselected")
                                        <div class="status-first">
                                            <div class="form-group {{ $key }}-checkbox">
                                                <input type="checkbox" name="{{ $checkbox_active }}" checked="checked"
                                                       id="checkbox_{{ $key }}" class="project_active_checkbox"
                                                       value="{{ $dem }}">
                                                <label for="{{ $key }}">{{ $value }}</label>
                                            </div>
                                        </div>
                                    @else
                                        @include('form.input.checkbox', ["name" => $checkbox_active, "id" => 'checkbox_'.$key, "label" => $value, "class" => "project_active_checkbox", "checked" => false, "value" => $dem, "onclick" => 'checkValue' , $data_id = $key ])
                                    @endif
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-4 col-6">
                                <div class="list-status-order">
                                    <div class="group-order">
                                        <div class="row">
                                            <div class="col-md-10 col-sm-10 col-10">
                                                @if($key == "unselected")
                                                    @include('form.input.number', ["name" => $checkbox_order, "id" => $key, "label" => $value, "checked" => false, "value" => "", "onchange" => "checkValue", "onkeyup" => "checkValue" ,'class' => 'project_status_order'])
                                                @else
                                                    @include('form.input.number', ["name" => $checkbox_order, "id" => $key, "label" => $value, "checked" => false, "value" => "", "onchange" => "checkValue", "onkeyup" => "checkValue" ,'class' => 'project_status_order'])
                                                @endif
                                            </div>
...
@section('scripts')
    {!! JsValidator::formRequest('Modules\User\Http\Requests\ProjectCreateRequest', '#form') !!}
    ...
    <script type="text/javascript">
        ...
        //push data
        var add_push = ["{{ \Illuminate\Support\Facades\Auth::guard(config("sys_auth.user"))->user()->email }}", "{{ config("sys_auth.auth_project")["admin"]["id"] }}", "{{ config("sys_auth.auth_project")["owner"]["name"] }}"];
        ...
        $("#add_permission").click(function () {
            ...
            $.ajax({
                ...
                method: "POST",
                url: "{{ route('user.project.check.permission.email') }}",
        ...
        function renderHtml(a) {
            ...
            for ($i = 0; $i < arr_permission.length; $i++) {
                if (typeof arr_permission[$i][0] != 'undefined' &&
                    ...
                    html += '<td>' + arr_permission[$i][2] + '</td>';
                    html += '<td width="30" class="text-right">';
                    if (arr_permission[$i][0] !== "{{ \Illuminate\Support\Facades\Auth::guard(config("sys_auth.user"))->user()->email }}") {
