...
namespace App\Repositories\User\ProjectPermission;
...
class ProjectPermissionRepository implements ProjectPermissionRepositoryInterface
{
    private $model;
    ...
    {
        $project_creator = $this->modelProject->where('creator', Auth::guard(config("sys_auth.user"))->user()->id)->pluck("id");
        return $this->model->where('user_id', Auth::guard(config("sys_auth.user"))->user()->id)
...
//            ->where("permission_id", config("sys_auth.auth_project")["admin"]["id"])->pluck("project_id");
    ...
    {
        $project_creator = $this->modelProject->where('creator', Auth::guard(config("sys_auth.user"))->user()->id)->pluck("id");
        $owner_project_permission = $this->model->where('user_id', Auth::guard(config("sys_auth.user"))->user()->id)
