...
class AdminTaskService
{
    ...
    public function __construct(AdminTaskRepositoryInterface $repository, MailService $mailService, AdminUserService $userService)
    {
    ...
    public function store($data)
    {
        ...
        $data['admin_id'] = Auth::guard(config("sys_auth.admin"))->user()->id;
    ...
    public function listTaskForCategory($data = [])
    {
        ...
        foreach ($arr_category as $k => $category) {
            $tasks = $this->getListTaskForCategory(["category_name" => $k, "project_id" => (!empty($user->project_admin_actived) ? $user->project_admin_actived : ""), "month" => (int)$data["month"], "year" => $data["year"]]);
            foreach ($tasks as $task) {
                $task["point_start"] = Helpers::taskPoint($task, $data, "point_start");
                $task["point_end"] = Helpers::taskPoint($task, $data, "point_end");
...
//            //check
//            if (!empty($row->start_date) && !empty($row->end_date)) {
//                if ($data["month"] >= $start_date_m && $data["month"] <= $end_date_m) {
//
//                } elseif ($data["month"] >= $start_date_m && $end_date_m >= $data["month"]) {
//
//                } else {
//                    unset($tasks{$k});
//                }
//            } elseif (!empty($row->start_date) && empty($row->end_date)) {
//                if ($start_date_m > $data["month"]) {
//                    unset($tasks{$k});
//                }
//            } elseif (empty($row->start_date) && !empty($row->end_date)) {
